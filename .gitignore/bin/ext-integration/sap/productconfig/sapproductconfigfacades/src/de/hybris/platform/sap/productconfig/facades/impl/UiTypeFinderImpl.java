...
public class UiTypeFinderImpl implements UiTypeFinder
{
	...
	protected boolean isCheckboxList(final CsticModel model, final boolean isDebugEnabled, final boolean hasValueImages)
	{
		...
		isCheckboxList = isMultiSelection(model, isDebugEnabled) && isConstrainedOrHasAssignableValues(model, isDebugEnabled)
				&& model.getStaticDomainLength() > 1 && !hasValueImages;
	...
	protected boolean isInput(final CsticModel model, final boolean isDebugEnabled)
	{
		...
		if (isValueTypeSupported(model, isDebugEnabled) && editableWithoutAdditionalValue(model, isDebugEnabled)
				&& !model.isMultivalued())
		{
