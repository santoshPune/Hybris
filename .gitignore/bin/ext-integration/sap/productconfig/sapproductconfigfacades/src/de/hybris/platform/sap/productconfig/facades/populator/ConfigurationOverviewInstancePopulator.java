		...
		implements ConfigurablePopulator<InstanceModel, List<CharacteristicGroup>, Map>
{
	...
	public static final String HAS_ONLY_ONE_CSTIC_GROUP = "HAS_ONLY_ONE_CSTIC_GROUP";
	public static final String APPLIED_CSTIC_FILTERS = "APPLIED_FILTERS";
	public static final String APPLIED_GROUP_FILTERS = "APPLIED_GROUP_FILTERS";
	public static final String HYBRIS_NAME_MAP = "HYBRIS_NAME_MAP";
...

	{
		...
		final Set<String> groupsToBeDisplayed = overviewGroupFilter.getGroupsToBeDisplayed(source, filteredOutGroups);
		final Map<String, ClassificationSystemCPQAttributesContainer> nameMap = (HashMap<String, ClassificationSystemCPQAttributesContainer>) optionsMap
				.get(HYBRIS_NAME_MAP);
	...
	protected void createCsticValues(final List<CharacteristicValue> values, final CsticModel cstic,
			final List<CsticValueModel> filteredAssignedValues,
			final Map<String, ClassificationSystemCPQAttributesContainer> nameMap)
	{
		final ClassificationSystemCPQAttributesContainer cpqAttributes = nameProvider.getCPQAttributes(cstic.getName(), nameMap);
		...
		for (int i = 0; i < filteredAssignedValues.size(); i++)
		{
			...
			configurationOverviewValuePopulator.populate(csticValue, value, options);
			values.add(value);
		}
	}
	...
	protected List<CsticValueModel> applyFilters(final CsticModel cstic, final List<ConfigOverviewFilter> filters)
	{
		...
		final List<CsticValueModel> visibleAssignedValues = visibleValueFilter.filter(cstic.getAssignedValues(), cstic);
	...
	protected String retrieveHibrisProductName(final String name)
	{
		...
		final List<ProductModel> products = productDao.findProductsByCode(name);
