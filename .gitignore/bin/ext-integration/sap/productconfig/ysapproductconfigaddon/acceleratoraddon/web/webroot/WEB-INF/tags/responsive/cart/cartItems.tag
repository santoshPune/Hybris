 ...
 <ul class="item-list cart-list">
    <li class="hidden-xs hidden-sm">
        <ul class="item-list-header">
            ...
            <li class="item-delivery"><spring:theme code="basket.page.delivery"/></li>
    ...
    <c:forEach items="${cartData.entries}" var="entry" varStatus="loop">
     ...
     <c:if test="${not entry.configurationConsistent and entry.configurationAttached}">
           <li class="product-item cpq-cart-error-panel">     
              <div class="row" >
                  <div class="cpq-cart-error">
                 	<div class="cpq-cart-error-sign">&#xe101;</div>
         			 <div class="cpq-cart-error-message">
                    <span><spring:message code="sapproductconfig.cart.entrytext.conflicts.responsive" 
                     text="{0} issues must be resolved before checkout" arguments="${entry.configurationErrorCount}" />&nbsp;
                    <spring:url value="${entry.itemPK}/ ${entry.product.code} /configCartEntry" var="configUrl"></spring:url>
                    <a  href="${configUrl}"><spring:message code="sapproductconfig.addtocart.resolve.button" text="Resolve Issues Now" /></a>
                   </span>
	                   </div>
	               </div>
                </div>
        ...
        <li class="item-list-item <c:if test="${not entry.configurationConsistent and entry.configurationAttached}">cpq-cart-error-item</c:if>">
            ...
            <div class="item-info">
                ...
                <div class="item-stock">
                    ...
                    <c:if test="${not entry.configurationAttached}"> 
                ...
                <%-- configure link --%>
                 <c:if test="${entry.configurationAttached}">
					   <div class="cpq-cart-config">
				      	<spring:url	value="${entry.itemPK}/ ${entry.product.code} /configCartEntry"	var="configUrl"></spring:url>
				        <br> <a href="${configUrl}"><spring:theme code="sapproductconfig.cart.link"/></a>	
				     </div>	
		          </c:if>
                ...
                <c:if test="${entry.product.purchasable}">
                    <c:if test="${not empty entryStock and entryStock ne 'outOfStock'}">
                        <c:if test="${entry.deliveryPointOfService eq null or not entry.product.availableForPickup}">
                            <span class="item-delivery-label"><spring:theme code="basket.page.shipping.ship"/></span>
                        </c:if>
                    </c:if>
                    <c:if test="${not empty entry.deliveryPointOfService.name}">
                        <span class="item-delivery-label"><spring:theme code="basket.page.shipping.pickup"/></span>
                    </c:if>

                    <c:if test="${entry.product.availableForPickup and not empty entry.deliveryPointOfService.name}">
                        <div class="item-delivery-store">${entry.deliveryPointOfService.name}</div>
                    </c:if>
                </c:if>
            </div>
