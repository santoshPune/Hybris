...
public class ConfigurationProviderSSCImpl extends BaseConfigurationProviderSSCImpl
{
			...
			final Map<String, String> csticValueAuthorMap) throws IpcCommandException
	{
		...
		if (LOG.isDebugEnabled())
		{
			LOG.debug("created CsticModel from ICsticData: " + csticModels);
		}
			...
			final Map<String, String> csticValueAuthorMap)
	{
		...
		{
			final CsticValueModel csticValueModel = createModelValue(csticData, csticValueData);
	...
	protected InstanceModel createInstance(final IInstanceData instanceData)
	{
		...
		if (LOG.isDebugEnabled())
		{
			LOG.debug("created InstanceModel from IInstanceDatal: " + instanceModel.toString());
		}

		return instanceModel;
	}

	protected CsticValueModel createModelValue(final ICsticData csticData, final ICsticValueData csticValueData)
	{
		final int type = csticData.getCsticHeader().getCsticValueType().intValue();
		final CsticValueModel csticValueModel = getConfigModelFactory().createInstanceOfCsticValueModel(type);
			...
			final IConfigSession session, final ICsticData[] csticsData) throws IpcCommandException
	{
		...
		final List<CsticGroupModel> csticGroupModelList = new ArrayList<>(csticGroups.length);

		final Map<String, CsticGroupModel> csticGroupModelMap = new HashMap<>();
		final Map<String, Map<Integer, String>> csticOrderInGroupMap = new HashMap<>();

		prepareGroupMaps(csticGroups, csticGroupModelList, csticGroupModelMap, csticOrderInGroupMap);
		...
		for (final ICsticData csticData : csticsData)
		{
			...
			else
			{
				...
				for (final String groupName : groupNames)
				{
					...
					// if a cstic is assigned to more than one group, ssc api
					// provide only the cstic position in the first group,
					// therefore we use the cstic instance position to place
					// such cstic in the group; we place them at the end of the
					// group
		...
		for (final CsticGroupModel csticGroupModel : csticGroupModelList)
		{
			...
			final Map<Integer, String> treeMap = new TreeMap<>(csticMap);
			final List<String> csticList = new ArrayList<>(treeMap.values());
	...
	protected void prepareGroupMaps(final ICsticsGroups[] csticGroups, final List<CsticGroupModel> csticGroupModelList,
			final Map<String, CsticGroupModel> csticGroupModelMap, final Map<String, Map<Integer, String>> csticOrderInGroupMap)
	{
	...
	{
		final Map<String, String> csticValueAuthorMap = new HashMap<>();
		try
		{
	...
	protected Map<String, List<IInstanceData>> retrieveInstances(final IConfigSession session, final String configId)
			throws IpcCommandException
	{
		final Map<String, List<IInstanceData>> instanceMap = new HashMap<>();
