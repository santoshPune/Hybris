...
public class DefaultSAPRecommendationItemDSUIEditor extends AbstractTextBasedUIEditor
{
			...
			final EditorListener listener)
	{
		...
		if (isEditable())
		{
			...
			catch (ODataException | URISyntaxException | IllegalArgumentException e)
			{
				LOG.debug("Error filling item datasource types dropdown");
				showMesssagePopupWithDetail(e);
			}
			catch (IOException e)
			{
				LOG.debug("Error reaching backend system");
				showMesssagePopupWithDetail(e);
			}
	...
	protected SAPRecommendationItemDSTypeReader getItemDSTypeReader()
	{
		...
		return recommendationItemDSReader = (SAPRecommendationItemDSTypeReader) Registry.getApplicationContext()
				.getBean("sapRecommendationItemDSTypeReader");
	}
	...
	private void showMesssagePopupWithDetail(final Exception e)
	{
		try
		{
			...
			if(e.getClass().toString().contains("Configuration"))
			{
		...
		{
			LOG.error("Messagebox Exception");
		}		
	}
}
