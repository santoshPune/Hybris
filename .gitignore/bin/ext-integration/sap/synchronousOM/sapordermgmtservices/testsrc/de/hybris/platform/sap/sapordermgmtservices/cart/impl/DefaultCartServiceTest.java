...
public class DefaultCartServiceTest
{
	...
	private final CartEntryConfigurationAttributes configAttributes = new CartEntryConfigurationAttributes();
	...
	public void init()
	{
		...
		EasyMock
				.expect(
						productConfigurationService.calculateCartEntryConfigurationAttributes(EasyMock.anyObject(String.class),
								EasyMock.anyObject(String.class), EasyMock.anyObject(String.class)))//
				.andReturn(configAttributes).anyTimes();

		configAttributes.setConfigurationConsistent(Boolean.FALSE);
		configAttributes.setNumberOfErrors(Integer.valueOf(1));
		...
		EasyMock.replay(bolCartFacade, productConfigurationService);
	...
	private Converter<Item, OrderEntryData> createCartItemConverter()
	{
		final Converter<Item, OrderEntryData> converter = new AbstractConverter<Item, OrderEntryData>()
		{
			...
			public void populate(final Item source, final OrderEntryData target)
			{
				...
				populator.setProductConfigurationService(productConfigurationService);
				populator.setPriceFactory(createPriceFactory());
				populator.populate(source, target);
	...
	@Test
	public void testAddToCartEntryConfigurable()
	{
		newItem.setConfigurable(true);
		final CartModificationData modification = classUnderTest.addToCart(productId, quantity);
		assertNotNull(modification);
		final OrderEntryData entry = modification.getEntry();
		assertNotNull(entry);
