...
public class DefaultCartRestorationFacade implements CartRestorationFacade
{
	...
	protected void addCartEntriesToStandardCart(final CartData newCart) throws CommerceCartModificationException
	{
		...
		for (final OrderEntryData entry : newCart.getEntries())
		{
			...
			if (productConfigurationService.isInSession(entry.getItemPK()))
			{
				entryModel.setExternalConfiguration(productConfigurationService.getExternalConfiguration(entry.getItemPK()));
				entryModel.setBasePrice(productConfigurationService.getTotalPrice(entry.getItemPK()));
