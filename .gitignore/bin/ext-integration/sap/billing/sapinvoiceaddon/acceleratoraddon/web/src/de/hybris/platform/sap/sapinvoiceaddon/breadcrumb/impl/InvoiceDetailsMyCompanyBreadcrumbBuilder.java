...
public class InvoiceDetailsMyCompanyBreadcrumbBuilder {
	...
	public List<Breadcrumb> createAccountSummaryBreadcrumbs() {
		...
		if (configurationService.getConfiguration()
				.getString(SapinvoiceaddonConstants.ACCOUNT_STATUS_UI_VERSION)
				.equals(SapinvoiceaddonConstants.ACCOUNT_STATUS_DESKTOP_STRING)) {
			breadcrumbs
					.add(new Breadcrumb(
							SapinvoiceaddonConstants.ACCOUNT_STATUS_PATH,
							getMessageSource()
									.getMessage(
				...
				.getString(SapinvoiceaddonConstants.ACCOUNT_STATUS_UI_VERSION)
				.equals(SapinvoiceaddonConstants.ACCOUNT_STATUS_RESPONSIVE_STRING)) {
			breadcrumbs
					.add(new Breadcrumb(
	...
	public List<Breadcrumb> createInvoiceDetailsBreadcrumbs(
		...
		switch (configurationService.getConfiguration().getString(
				SapinvoiceaddonConstants.ACCOUNT_STATUS_UI_VERSION)) {
		case SapinvoiceaddonConstants.ACCOUNT_STATUS_DESKTOP_STRING: {
			if (b2bUnitUID != null) {
				breadcrumbs
						.add(new Breadcrumb(
								String.format(
										SapinvoiceaddonConstants.ACCOUNT_STATUS_DOCUMENTS_PATH,
										b2bUnitUID),
								getMessageSource()
										.getMessage(
		...
		case SapinvoiceaddonConstants.ACCOUNT_STATUS_RESPONSIVE_STRING: {
			if (b2bUnitUID != null) {
				breadcrumbs
						.add(new Breadcrumb(
								String.format(
