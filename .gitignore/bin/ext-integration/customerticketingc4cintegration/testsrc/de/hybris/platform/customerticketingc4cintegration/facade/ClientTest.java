...
public class ClientTest extends ServicelayerTest
{
	...
	public void testForGetTickets1()
	{
		...
		for (TicketData t : tickets.getResults())
		{
			LOGGER.info("ticket is " + t.getId());
			LOGGER.info("ticket is " + t.getSubject());
		}
		LOGGER.info(tickets.getPagination().getNumberOfPages());
		LOGGER.info(tickets.getPagination().getTotalNumberOfResults());
		LOGGER.info(tickets.getPagination().getCurrentPage());
		LOGGER.info(tickets.getPagination().getPageSize());
	...
	public void testForGetTickets3()
	{
		...
		LOGGER.info(tickets.getPagination().getCurrentPage());
		LOGGER.info(tickets.getPagination().getPageSize());
		LOGGER.info(tickets.getResults().size());
		assertFalse(tickets.getResults().isEmpty());
	}
	...
	public void testForUpdateTicket()
	{
		...
		assertTrue(data.getTicketEvents().stream().map(event -> event.getText()).collect(Collectors.joining("")).contains("EASYSHARE-Z730-Zoom-Digital-Camera"));
		assertTrue(data.getTicketEvents().stream().map(event -> event.getText()).collect(Collectors.joining("")).contains("Let me take a look at this product"));
