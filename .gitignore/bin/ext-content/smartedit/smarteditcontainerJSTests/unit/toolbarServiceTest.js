...
describe('test outer toolbarService Module', function() {
    ...
    it('adding 2 actions with the no priority gives them the same default priority and triggers a warning', function() {
        ...
        expect($log.warn.calls[0].args[0]).toContain('WARNING: In toolBar1 the items ');
        expect($log.warn.calls[0].args[0]).toContain('key2');
        expect($log.warn.calls[0].args[0]).toContain('key1');
        expect($log.warn.calls[0].args[0]).toContain('have the same priority');
