...
public class NamedQueryServiceTest
{
	...
	{
		FlexibleSearchQuery flexibleSearchQuery = Mockito.mock(FlexibleSearchQuery.class);
	...
	{
		final NamedQuery namedQuery = new NamedQuery().withSort(new ArrayList<>()).withParameters(new HashMap<>()).withPageSize(
				100).withCurrentPage(1).withQueryName(QUERY_NAME);
