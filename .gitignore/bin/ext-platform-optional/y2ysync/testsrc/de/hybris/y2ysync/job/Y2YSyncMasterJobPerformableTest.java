...
public class Y2YSyncMasterJobPerformableTest extends ServicelayerBaseTest
{
	...
	public void shouldWaitWithSyncProcessWithNotMatchingNodeGroup() throws Exception
	{
		// given
		final Y2YSyncJobModel job = syncExecutionService.createSyncJobForZip("zipTestSyncJob", zipSyncContainer);
		final String notMatchingNodeGroup = "badGroup";
		job.setNodeGroup(notMatchingNodeGroup);
		modelService.save(job);

		assertThat(findCurrentTasksForNodeGroup(job.getNodeGroup())).isEmpty();
		...
		assertThat(foundMediaData).isNull();
		assertThat(foundMediaBinaries).isNull();
		assertThat(findCurrentTasksForNodeGroup(job.getNodeGroup())).isNotEmpty();// tasks are waiting in db for matching node
	}
	...
	private List<TaskModel> findCurrentTasksForNodeGroup(final String nodeGroup)
	{
		final String query = "SELECT {PK} FROM {" + TaskModel._TYPECODE + "} WHERE {" + TaskModel.NODEGROUP + "}=?nodeGroup";
		final FlexibleSearchQuery fQuery = new FlexibleSearchQuery(query, ImmutableMap.of("nodeGroup", nodeGroup));

		final List<TaskModel> tasks = flexibleSearchService.<TaskModel> search(fQuery).getResult();
		return tasks;
	}
