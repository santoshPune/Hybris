...
public class ItemChangesProcessorIntegrationTest extends ServicelayerBaseTest
{
	...
	private static final String MEDIA_PK_KEY = SyncTaskFactory.MEDIA_PK_KEY;
	private static final String CONDITION_NAME_KEY = SyncTaskFactory.CONDITION_NAME_KEY;
	private static final String IMPEX_HEADER_KEY = SyncTaskFactory.IMPEX_HEADER_KEY;
	private static final String TYPE_CODE_KEY = SyncTaskFactory.TYPE_CODE_KEY;
	private static final String SYNC_EXECUTION_ID_KEY = SyncTaskFactory.SYNC_EXECUTION_ID_KEY;
	private static final String SYNC_TYPE_KEY = SyncTaskFactory.SYNC_TYPE_KEY;
	...
	public void testRunSuccessfully() throws Exception
	{
		...
		final MediasForType mediasForType1 = MediasForType.builder().withComposedTypeCode(titleComposedType.getCode())
	...
	public void testWaitWhenNoMatchingNodeGroupGiven() throws Exception
	{
		...
		finally
		{
			...
			//task still waiting in db
			assertThat(task.getPk()).isNotNull();
			modelService.refresh(task);
			assertThat(task).isNotNull();
			assertThat(task.getNodeGroup()).isEqualTo("badGroup");
		}
	}
	...
	public void testRunWithErrorsDuringChangeConsumption() throws Exception
	{
		...
		try
		{
			TestUtils.disableFileAnalyzer(
					"de.hybris.y2ysync.task.runner.ItemChangesProcessorIntegrationTest.testRunWithErrorsDuringChangeConsumption()");
	...
	private Map<String, Object> getTaskContext(final String syncExecutionId, final PK mediaPK, final MediasForType mediasForType)
	{
		final ImmutableMap.Builder<String, Object> result = ImmutableMap.<String, Object> builder()
				...
				.put(SYNC_TYPE_KEY, Y2YSyncType.ZIP).put(TYPE_CODE_KEY, mediasForType.getComposedTypeCode());//
		return result.build();
	}
