...
public class DefaultCouponManagementServiceUnitTest
{
	...
	public void testValidateSingleCodeCoupon()
	{
		...
		when(findSingleCodeCouponStrategy.findCouponForCouponCode(COUPON_ID)).thenReturn(Optional.of(singleCodeCoupon));
		when(
				Boolean.valueOf(singleCodeCouponRedemptionStrategy.isRedeemable(any(SingleCodeCouponModel.class),
						any(AbstractOrderModel.class), anyString()))).thenReturn(Boolean.TRUE);
	...
	public void testNotValidSingleCodeCoupon()
	{
		...
		when(
				Boolean.valueOf(singleCodeCouponRedemptionStrategy.isRedeemable(any(SingleCodeCouponModel.class),
						any(AbstractOrderModel.class), anyString()))).thenReturn(Boolean.FALSE);
