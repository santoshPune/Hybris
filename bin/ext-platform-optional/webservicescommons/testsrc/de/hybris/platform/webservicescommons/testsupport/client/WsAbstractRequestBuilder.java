...
public abstract class WsAbstractRequestBuilder<T extends WsAbstractRequestBuilder<?>>
{
	...
	{
		return Registry.getCurrentTenant().getConfig().getString(WEBSERVICES_REQUIRED_CHANNEL_CONFIG_KEY, "https").equals("https") ? true
				: false;
	}
	...
	protected Client createClient()
	{
		try
		{
			final JacksonJsonProvider provider = new JacksonJaxbJsonProvider().configure(
			...
			return ClientBuilder.newBuilder().withConfig(config).hostnameVerifier(new DummyHostnameVerifier()).sslContext(sc)
					.build();
		}
