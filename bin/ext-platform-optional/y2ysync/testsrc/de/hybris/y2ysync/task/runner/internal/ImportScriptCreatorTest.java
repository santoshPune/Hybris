...
public class ImportScriptCreatorTest extends ServicelayerBaseTest
{
	...
	{
		return scripts.stream().map(TestableImportScript::new).filter(TestableImportScript::isRemoveScript).findFirst()
				.orElse(null);
	}
	...
	private ImportScriptCreator givenImportScriptCreator(final ItemChangeDTO... changes)
	{
		...
		return new ImportScriptCreator(modelService, exportService, exportScriptCreator, userService);
	}
