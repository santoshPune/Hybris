...
public class DefaultCustomizationFacadeManagingTransactionMethodsTest extends ServicelayerBaseTest
{
	...
	{
		//given
		final CustomizationData customizationData = creteCustomizationData(NEW_CUSTOMIZATION_ID, NEW_CUSTOMIZATION_NAME, NEW_VARIATION_ID, NEW_VARIATION_NAME, null, null);
	...
	public void updateCustomizationWithRelatedObjectsTest()
	{
		...
		final boolean newActiveValue = !variation.isActive();
		variation.setActive(newActiveValue);
		...
		Assert.assertTrue("Variation should be updated", newActiveValue == variation.isActive());
	...
	{
		//given
		customizationFacade.createCustomization(creteCustomizationData(CUSTOMIZATION_ID, CUSTOMIZATION_NAME, null, null, null, null), CATALOG_ID,
				CATALOG_VERSION_STAGE_ID);

		final CustomizationData customizationData = creteCustomizationData(CUSTOMIZATION_ID, CUSTOMIZATION_NAME, NEW_VARIATION_ID, NEW_VARIATION_NAME, NEW_TRIGGER_ID,
				SEGMENT_ID);
	...
	public void updateCustomizationWithDuplicatedVariatonTest()
	{
		...
		customizationFacade.createCustomization(creteCustomizationData(CUSTOMIZATION_ID, CUSTOMIZATION_NAME, null, null, null, null), CATALOG_ID,
				CATALOG_VERSION_STAGE_ID);
		final CustomizationData customizationData = creteCustomizationData(CUSTOMIZATION_ID, CUSTOMIZATION_NAME, VARIATION_ID, VARIATION_NAME, null, null);
		customizationData.setDescription(NEW_DESCRIPTION);
		final VariationData variation = customizationData.getVariations().get(0);
		customizationData.setVariations(Arrays.asList(variation, variation));
	...
	public void updateCustomizationWhithIncorrectTriggerTest()
	{
		...
		customizationFacade.createCustomizationWithRelatedObjects(
				creteCustomizationData(CUSTOMIZATION_ID, CUSTOMIZATION_NAME, VARIATION_ID, VARIATION_NAME, null, null), CATALOG_ID, CATALOG_VERSION_STAGE_ID);

		final CustomizationData customizationData = creteCustomizationData(CUSTOMIZATION_ID, CUSTOMIZATION_NAME, VARIATION_ID, VARIATION_NAME, NEW_TRIGGER_ID,
				NOT_EXISTING_SEGMENT_ID);
		customizationData.setDescription(NEW_DESCRIPTION);
		final boolean newEnabledValue = !customizationData.getVariations().get(0).isEnabled();
		customizationData.getVariations().get(0).setEnabled(newEnabledValue);
		...
		Assert.assertEquals("Variations size should be 1", 1, foundCustomization.getVariations().size());
		Assert.assertFalse("Variation should not be updated",
				newEnabledValue == foundCustomization.getVariations().get(0).isEnabled());
	}
}
