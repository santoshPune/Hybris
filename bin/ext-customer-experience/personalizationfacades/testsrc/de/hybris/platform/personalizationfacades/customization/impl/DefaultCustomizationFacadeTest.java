...
public class DefaultCustomizationFacadeTest
{
	...
	@Test
	public void getCustomizationsWithCodeTest()
	{
		//given
		final Map<String, String> queryParmas = Collections.singletonMap("code", CUSTOMIZATION_ID);
		Mockito.when(catalogVersionService.getCatalogVersion(CATALOG_ID, CATALOG_VERSION_STAGE_ID)).thenReturn(catalogVersionStage);
		Mockito.when(customizationService.getCustomizations(catalogVersionStage, queryParmas, 0, 1))
				.thenReturn(new SearchResultImpl<>(Collections.singletonList(customization), 10, 1, 0));
		...
		//when
		final SearchResult<CustomizationData> resultList = customizationFacade.getCustomizations(CATALOG_ID,
				CATALOG_VERSION_STAGE_ID, Collections.singletonMap("code", CUSTOMIZATION_ID), 0, 1);

		//then
		Assert.assertNotNull(resultList);
		Assert.assertNotNull(resultList.getResult());
		Assert.assertEquals(1, resultList.getResult().size());
		final CustomizationData result = resultList.getResult().get(0);
		Assert.assertEquals(CUSTOMIZATION_ID, result.getCode());
	}
	...
	public void createCustomizationsTest()
	{
		...
		Mockito.when(customizationService.isDefaultGroup(catalogVersionStage)).thenReturn(true);
	...
	public void createCustomizationsTestForNoCustomizationGroup()
	{
		...
		Mockito.when(catalogVersionService.getCatalogVersion(CATALOG_ID, CATALOG_VERSION_STAGE_ID)).thenReturn(catalogVersionStage);
		Mockito.when(customizationService.getCustomization(NEW_CUSTOMIZATION_ID, catalogVersionStage)).thenReturn(Optional.empty());
		Mockito.when(customizationService.isDefaultGroup(catalogVersionStage)).thenReturn(false);
		Mockito.when(customizationReverseConverter.convert(customizationData)).thenReturn(customization);
		Mockito.when(customizationService.createCustomization(Mockito.eq(customization), Mockito.any(),
				Mockito.eq(customization.getRank()))).thenReturn(customization);
		Mockito.when(customizationConverter.convert(Mockito.any(), Mockito.anyList())).thenReturn(customizationData);
		...
		//then
		final CxCustomizationsGroupModel createdCustomizationGroup = Mockito.spy(customizationFacade).getOrCreateCustomizationGroup(catalogVersionStage);
