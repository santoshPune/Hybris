...
public class DefaultSegmentFacadeTest
{
	...
	{
		//given
		Mockito.when(segmentService.getSegments(Mockito.anyMap(), Mockito.anyInt(), Mockito.anyInt())).thenReturn(searchResult);
		Mockito.when(segmentConverter.convertAll(Collections.singletonList(segment), SegmentConversionOptions.FULL))
				.thenReturn(Collections.singletonList(segmentData));
		...
		final SegmentData segment = resultList.get(0);
		Assert.assertEquals(SEGMENT_ID, segment.getCode());
	}
