...
describe('seMediaUploadForm', function() {
    ...
    beforeEach(module('pascalprecht.translate', function($translateProvider) {
        $translateProvider.translations('en', {
            'upload.image.cancel': 'Cancel',
            'upload.image.submit': 'Upload',
            'upload.image.replace': 'Replace Image',
            'uploaded.image.description': 'Description',
            'uploaded.image.code': 'Code',
            'uploaded.image.alt.text': 'Alt Text',
            'uploaded.is.uploading': 'Uploading...'
        });
    ...
    beforeEach(module('seObjectValidatorFactoryModule', function($provide) {
        ...
        seObjectValidatorFactory.build.andReturn(validator);
    ...
    describe('on upload with invalid parameters', function() {
        beforeEach(function() {
            validator.validate.andCallFake(function(objectToValidate, errorsContext) {
                errorsContext.push({
                    subject: 'code',
    ...
    describe('on upload', function() {
        ...
        beforeEach(function() {
            validator.validate.andReturn(true);
            uploadMediaDeferred = $q.defer();
            seMediaService.uploadMedia.andReturn(uploadMediaDeferred.promise);
            element.find('.se-media-upload-btn__submit').click();
        });
        ...
        describe('on failure', function() {
            beforeEach(function() {
                seBackendValidationHandler.handleResponse.andCallFake(function(response, errorsContext) {
                    errorsContext.push({
                        subject: 'code',
