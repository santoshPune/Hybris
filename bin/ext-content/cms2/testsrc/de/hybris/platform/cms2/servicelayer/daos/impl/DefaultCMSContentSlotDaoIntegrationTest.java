...
public class DefaultCMSContentSlotDaoIntegrationTest extends ServicelayerTransactionalTest
{
	...
	@Test
	public void shouldFindTemplateRelationsForContentSlot()
	{
		// MiddleContentSlot is used in 2 templates and not overridden in any pages
		final Collection<ContentSlotForTemplateModel> relations = cmsContentSlotDao
				.findTemplateRelationsByContentSlotNotOverriddenInPage(getContentSlotById(MIDDLE_CONTENT_SLOT), catalogVersion);

		assertThat(relations.size(), is(2));
	}

	@Test
	public void shouldFindTemplateRelationForContentSlot()
	{
		// TopContentSlot is used in 2 templates, but some pages override this slot
		final Collection<ContentSlotForTemplateModel> relations = cmsContentSlotDao
				.findTemplateRelationsByContentSlotNotOverriddenInPage(getContentSlotById(TOP_CONTENT_SLOT), catalogVersion);

		assertThat(relations.size(), is(1));
	}

	@Test
	public void shouldFindTemplateRelationForContentSlot_Empty()
	{
		final Collection<ContentSlotForTemplateModel> relations = cmsContentSlotDao
				.findTemplateRelationsByContentSlotNotOverriddenInPage(getContentSlotById(PUNCHED_OUT_CONTENT_SLOT), catalogVersion);

		assertThat(relations, empty());
	}

	@Test
	public void shouldFindTemplateRelationsForContentSlot_MultipleTemplatesZeroPage()
	{
		final Collection<ContentSlotForTemplateModel> relations = cmsContentSlotDao
				.findTemplateRelationsByContentSlotWithoutPage(getContentSlotById(BOTTOM_CONTENT_SLOT), catalogVersion);
