...
import de.hybris.platform.cmswebservices.common.facade.validator.impl.DefaultLocalizedValidator;
...
import de.hybris.platform.cmswebservices.items.facade.validator.consumer.LocalizedFormatMediaAttributeValidationConsumer;
import de.hybris.platform.cmswebservices.items.facade.validator.consumer.LocalizedFormatStringAttributeValidationConsumer;
...
public class SimpleResponsiveBannerComponentValidatorTest
{
	...
	public void setup()
	{
		...
		when(cmsRequiredMediaFormatQualifiers.contains(FORMAT_WIDESCREEN)).thenReturn(true);
		when(validStringLengthPredicate.test(URL_LINK)).thenReturn(Boolean.TRUE);
	...
	public void shouldValidateAllMediaFormatsAndCodesMultipleLanguages()
	{
		...
		locales.put(ENGLISH, true);
		locales.put(FRENCH, true);

		final SimpleResponsiveBannerComponentData data = createComponentData(locales);
	...
	public void shouldFailAllMediaCodesNull()
	{
		...
		assertEquals(CmswebservicesConstants.FIELD_MEDIA_FORMAT_REQUIRED_L10N, errors.getFieldError().getCode());
	}
	...
	public void shouldValidateUrlLink_InvalidLength()
	{
		...
		locales.put(ENGLISH, true);
		locales.put(FRENCH, true);
		when(validStringLengthPredicate.test(URL_LINK)).thenReturn(Boolean.FALSE);
		final SimpleResponsiveBannerComponentData data = createComponentData(locales);
		final Errors errors = new BeanPropertyBindingResult(data, data.getClass().getSimpleName());
	...
	private SimpleResponsiveBannerComponentData createComponentData(final Map<String, Boolean> locales)
	{
	...
	private SimpleResponsiveBannerComponentData createComponentDataNoMediaCodes()
	{
		final Map<String, Map<String, String>> localizedMediaValues = new HashMap<>();
		localizedMediaValues.put(ENGLISH, mediaCodePerFormatMap);
