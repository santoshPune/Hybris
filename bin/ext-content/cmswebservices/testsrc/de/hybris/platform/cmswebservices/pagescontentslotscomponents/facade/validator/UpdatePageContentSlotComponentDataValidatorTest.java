 ...
 */
package de.hybris.platform.cmswebservices.pagescontentslotscomponents.facade.validator;
...
public class UpdatePageContentSlotComponentDataValidatorTest
{
	...
	public void setUp()
	{
		...
		when(componentExistsPredicate.test(INVALID)).thenReturn(Boolean.FALSE);
		...
		when(contentSlotExistsPredicate.test(INVALID)).thenReturn(Boolean.FALSE);
		...
		when(validationDtoFactory.buildComponentAndContentSlotValidationDto(Mockito.any(), Mockito.any())).thenReturn(
