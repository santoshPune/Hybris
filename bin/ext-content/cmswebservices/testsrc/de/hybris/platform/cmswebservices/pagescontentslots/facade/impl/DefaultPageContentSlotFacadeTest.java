 ...
 */
package de.hybris.platform.cmswebservices.pagescontentslots.facade.impl;
...
import de.hybris.platform.cmswebservices.pagescontentslots.facade.converter.ContentSlotDataConverter;
import de.hybris.platform.cmswebservices.pagescontentslots.service.PageContentSlotConverterRegistry;
import de.hybris.platform.cmswebservices.pagescontentslots.service.PageContentSlotConverterType;
import de.hybris.platform.cmswebservices.pagescontentslots.service.impl.DefaultPageContentSlotConverterType;
...
public class DefaultPageContentSlotFacadeTest
{
	...
	private static final String PAGE_ID = "email-page";
	...
	@Mock
	private PageContentSlotData templatePageContentSlotData;
	@Mock
	private PageContentSlotData customPageContentSlotData;
	@Mock
	...
	private final ContentSlotForPageModel pageRelation = new ContentSlotForPageModel();
	private final ContentSlotForTemplateModel templateRelation = new ContentSlotForTemplateModel();
	private final CustomRelationModel customRelation = new CustomRelationModel();
	...
	public void shouldNotFindSlotId() throws CMSItemNotFoundException, ConversionException
	{
		pageContentSlotFacade.getPagesBySlotId(INVALID);
	}

	@Test(expected = ConversionException.class)
	public void shouldNotFindConverter() throws CMSItemNotFoundException, ConversionException
	{
		when(adminContentSlotService.getPageRelationsForSlot(headerSlot)).thenReturn(Arrays.asList(pageRelation));
		when(adminContentSlotService.getTemplateRelationsForSlot(headerSlot)).thenReturn(Arrays.asList(templateRelation));
		when(pageContentSlotConverterRegistry.getPageContentSlotConverterType(any())).thenReturn(Optional.ofNullable(null));

		pageContentSlotFacade.getPagesBySlotId(SLOT_ID);
	}

	@Test
	public void shouldFindPageRelations() throws CMSItemNotFoundException, ConversionException
	{
		when(adminContentSlotService.getPageRelationsForSlot(headerSlot)).thenReturn(Arrays.asList(pageRelation));
		when(adminContentSlotService.getTemplateRelationsForSlot(headerSlot)).thenReturn(Arrays.asList(templateRelation));
		when(converter.convert(pageRelation)).thenReturn(pageContentSlotData);
		when(converter.convert(templateRelation)).thenReturn(templatePageContentSlotData);

		final List<PageContentSlotData> pages = pageContentSlotFacade.getPagesBySlotId(SLOT_ID);
		assertThat(pages.size(), is(2));
		assertThat(pages, containsInAnyOrder(pageContentSlotData, templatePageContentSlotData));
	}

	@Test
	public void shouldFindNoPageRelation() throws CMSItemNotFoundException, ConversionException
	{
		final List<PageContentSlotData> pages = pageContentSlotFacade.getPagesBySlotId(SLOT_ID);

		assertThat(pages, empty());
		verifyZeroInteractions(pageContentSlotConverterRegistry);
	}

	@Test
	public void shouldFindPageRelationsForCustomTypeExtendingCMSRelationModel()
			throws CMSItemNotFoundException, ConversionException
	{
		when(adminContentSlotService.getPageRelationsForSlot(headerSlot)).thenReturn(Arrays.asList(pageRelation, customRelation));
		when(converter.convert(pageRelation)).thenReturn(pageContentSlotData);
		when(converter.convert(customRelation)).thenReturn(customPageContentSlotData);

		final List<PageContentSlotData> pages = pageContentSlotFacade.getPagesBySlotId(SLOT_ID);
		assertThat(pages.size(), is(2));
		assertThat(pages, containsInAnyOrder(pageContentSlotData, customPageContentSlotData));
	}
