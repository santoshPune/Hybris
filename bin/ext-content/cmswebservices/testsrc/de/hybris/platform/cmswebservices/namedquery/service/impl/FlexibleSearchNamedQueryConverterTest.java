...
public class FlexibleSearchNamedQueryConverterTest
{
	...
	{
		final NamedQueryConversionDto namedQuery = new NamedQueryConversionDto()
				.withQuery(QUERY).withNamedQuery(new NamedQuery().withQueryName(QUERY_NAME));
	...
	{
		final NamedQueryConversionDto namedQueryConversion = new NamedQueryConversionDto()
				.withQuery(QUERY).withNamedQuery( //
		...
		Assert.assertEquals((namedQueryConversion.getNamedQuery().getCurrentPage()) * namedQueryConversion.getNamedQuery().getPageSize(), flexibleSearchQuery.getStart());
	}
	...
	public void testWithoutSortingAndWithoutParamsRequestingCurrentPageOnly()
	{
		...
		Assert.assertEquals((namedQueryConversion.getNamedQuery().getCurrentPage()) * DEFAULT_PAGE_SIZE, flexibleSearchQuery.getStart());
	}
	...
	public void testWithSortingAndWithoutParamsWithDefaultPagingOptions()
	{
		...
		namedQueryConversion //
				.getNamedQuery()//
				.withQueryName(QUERY_NAME)
				.withSort(Arrays.asList(sort1, sort2));
		final FlexibleSearchQuery flexibleSearchQuery = flexibleSearchNamedQueryConverter.convert(namedQueryConversion);
