...
public class AtpFormulaIntegrationTest extends BaseAcceleratorSourcingIntegrationTest
{
	...
	public void setUp() throws InterruptedException, OrderCancelException
	{
		...
		stockLevels.NewStockLevel(products.Camera(),warehouses.Montreal(),MONTREAL_STOCK_QTY);
		stockLevels.NewStockLevel(products.Camera(),warehouses.Boston(),BOSTON_STOCK_QTY);
		...
		ConsignmentEntryModel consignmentEntryModel = order.getConsignments().iterator().next().getConsignmentEntries().iterator().next();

		ShrinkageEventModel shrinkageEventModel = new ShrinkageEventModel();
		shrinkageEventModel.setQuantity(SHRINKED_QTY);
		shrinkageEventModel.setConsignmentEntry(consignmentEntryModel);
		...
		wastageEventModel.setConsignmentEntry(consignmentEntryModel);
	...
	public void getDefault_Atp()
	{
		...
		checkGlobalAndLocalAtp(Long.valueOf(MONTREAL_STOCK_QTY - getAllocatedQuantity()) + getCancelQuantity() - SHRINKED_QTY - WASTED_QTY + INCREASED_QTY);
	}
	...
	{
		updateFormula(atpFormulas.customFormula(null, null, null, null, null, null, null));
	...
	{
		updateFormula(atpFormulas.customFormula(true, null, null, null, null, null, null));
	...
	{
		updateFormula(atpFormulas.customFormula(true, false, null, null, null, null, null));
	...
	{
		updateFormula(atpFormulas.customFormula(true, false, true, null, null, null, null));
	...
	{
		updateFormula(atpFormulas.customFormula(true, false, true, null, null, false, null));
	...
	{
		updateFormula(atpFormulas.customFormula(true, false, true, null, null, false, false));
		checkGlobalAndLocalAtp(Long.valueOf(MONTREAL_STOCK_QTY - getAllocatedQuantity() + getCancelQuantity() - SHRINKED_QTY - WASTED_QTY));
	}

	@Test
	public void getAtp_Available_Allocated_Cancelled_Shrinked_Wasted_Increase()
	{
		updateFormula(atpFormulas.customFormula(true, false, true, true, null, false, false));
		checkGlobalAndLocalAtp(Long.valueOf(MONTREAL_STOCK_QTY - getAllocatedQuantity() + getCancelQuantity() - SHRINKED_QTY - WASTED_QTY + INCREASED_QTY));
	}
	...
	{
		updateFormula(atpFormulas.customFormula(null, null, null, null, null, false, false));
		assertEquals( (Long.valueOf(- SHRINKED_QTY - WASTED_QTY)), commerceStockService.getStockLevelForProductAndBaseStore(products.Camera(), baseStores.NorthAmerica()));
	}
	...
	protected void checkGlobalAndLocalAtp(final Long expectedLocal)
	{
		...
		if(expectedLocal != 0L) expectedGlobal = expectedLocal + Long.valueOf(BOSTON_STOCK_QTY);

		assertEquals(expectedGlobal, globalAtp);
		assertEquals(expectedLocal, posAtp);
	...
	protected void updateFormula(final AtpFormulaModel atpFormulaModel)
	{
		...
		warehouseStockService.setAtpformula(atpFormulaModel);
	}
