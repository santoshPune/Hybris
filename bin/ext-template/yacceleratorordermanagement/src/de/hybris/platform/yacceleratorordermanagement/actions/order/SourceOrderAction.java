...
public class SourceOrderAction extends AbstractProceduralAction<OrderProcessModel>
{
	...
	public void executeAction(final OrderProcessModel process) throws RetryLaterException, Exception
	{
		...
		if (results != null)
		{
			...
			partialFulfillment = order.getEntries().stream().anyMatch(orderEntry -> ((OrderEntryModel) orderEntry).getQuantityUnallocated().longValue() > 0);
			failedFulfillment = order.getEntries().stream().allMatch(orderEntry -> ((OrderEntryModel) orderEntry).getQuantityAllocated().longValue() == 0);
		}
