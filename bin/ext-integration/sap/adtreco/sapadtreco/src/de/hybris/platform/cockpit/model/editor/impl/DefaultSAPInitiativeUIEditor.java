...
public class DefaultSAPInitiativeUIEditor extends AbstractTextBasedUIEditor
{
	...
	protected List<SAPInitiative> searchValues(final String newSearchString)
	{
		...
		{
			showErrorPopup();
			LOG.debug("Connection to backend system failed due to wrong URI syntax");
		}
		catch (final ODataException ex)
		{
			showErrorPopup();
			LOG.debug("HTTP Destination is not configured correctly");
		}
		catch (final IOException ex)
		{
			showErrorPopup();
		...
		{
			showErrorPopup();
			LOG.debug("Runtime Error occurred in the backend");
		}
	...
	private void showErrorPopup(){
		try
		{
			final String exceptionText = de.hybris.platform.util.localization.Localization
					.getLocalizedString("adt.connectionError.description");
			final String exceptionTitle = de.hybris.platform.util.localization.Localization
					.getLocalizedString("adt.connectionError.title");
