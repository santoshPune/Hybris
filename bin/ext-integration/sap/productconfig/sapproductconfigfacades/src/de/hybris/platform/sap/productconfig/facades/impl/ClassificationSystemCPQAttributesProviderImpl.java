...
public class ClassificationSystemCPQAttributesProviderImpl implements ClassificationSystemCPQAttributesProvider
{
			...
			final ClassificationSystemVersionModel systemVersion)
	{
		...
		else
		{
			...
			final Map<String, Collection<MediaModel>> csticMedia = extractCsticMediaFromAttributeModel(attr);
	...
	protected Map<String, Collection<MediaModel>> extractCsticMediaFromAttributeModel(final ClassificationAttributeModel attr)
	{
		Map<String, Collection<MediaModel>> csticMedia = Collections.emptyMap();
		...
		if (mediaContainer != null)
		{
			...
			{
				final int size = (int) (media.size() / 0.75 + 1);
				csticMedia = new HashMap<>(size);
				csticMedia.put(attr.getCode(), media);
			}
		}
	...
	@Override
	public List<ImageData> getCsticMedia(final String csticKey, final ClassificationSystemCPQAttributesContainer cpqAttributes)
	{
		List<ImageData> images = new ArrayList<>();
		final Map<String, Collection<MediaModel>> csticMedia = cpqAttributes.getCsticMedia();
		if (MapUtils.isNotEmpty(csticMedia))
		{
			final Collection<MediaModel> media = csticMedia.get(csticKey);
			if (!CollectionUtils.isEmpty(media))
			{
				images = convertMediaToImages(media, getCpqCsticImageFormatMapping().getCPQMediaFormatQualifiers());
			}
		}
		return images;
	}
	...
	public String getOverviewValueName(final CsticValueModel valueModel, final CsticModel csticModel,
			final ClassificationSystemCPQAttributesContainer cpqAttributes, final boolean isDebugEnabled)
	{
		if (UiType.NOT_IMPLEMENTED == uiTypeFinder.findUiTypeForCstic(csticModel))
		{
		...
		if (displayName == null)
		{
			displayName = valueFormatTranslator.format(csticModel, valueModel.getName());
			logValue(CSTIC_VALUE_MODEL, VALUE_NAME, valueModel.getName(), DISPLAY_NAME, MODEL_NAME, displayName, isDebugEnabled);
		}
		return displayName;
	}
	...
	/**
	 * @return the characteristic UI type finder
	 */
	...
	/**
	 * @return value format translator
	 */
	...
	/**
	 * @return the cpqCsticImageFormatMapping
	 */
	public CPQImageFormatMapping getCpqCsticImageFormatMapping()
	{
	...
	/**
	 * @return the cpqCsticValueImageFormatMapping
	 */
	public CPQImageFormatMapping getCpqCsticValueImageFormatMapping()
	{
