...
<c:if test="${fn:length(validationErrors) gt 0}">
	<span id="${key}.errors" class="cpq-error">
		<c:forEach var="message" items="${validationErrors}" varStatus="status">
			<span class=cpq-error-msg> <spring:message code="${message.code}" arguments="${message.args}"
		  ...
		  <c:set var="conflictFound" value="false" />
		   <c:forEach var="message" items="${warnings}" varStatus="status">
				<c:if test="${message.type != 'CONFLICT'}">
					<span id="${key}.warnings" class="cpq-warning">
						<span class="cpq-warning-msg"> <spring:message code="${message.code}" arguments="${message.args}"
								text="${message.message}" />
						</span>
					</span>
				</c:if>
				<c:if test="${message.type == 'CONFLICT' && !conflictFound}">
					...
					<span id="${key}.conflicts" class="cpq-conflict">
						<span class="cpq-error-sign">&#xe101;</span>
						<a class="cpq-conflict-link" href="#">
							<spring:message code="sapproductconfig.cstic.conflict.message" text="Conflict Detected - Resolve Issues Now" />
						</a>
					</span>
				</c:if>
			</c:forEach>			
