...
public class DataloaderInitialLoadItemAction implements CockpitAction<SAPConfigurationModel, String>
{
	...
	public ActionResult<String> perform(final ActionContext<SAPConfigurationModel> ctx)
	{
		...
		if (isDataLoadRunning())
		{
			...
			if (!stopJobResult)
			{
				...
				Messagebox.show(ctx.getLabel("text.sapproductconfig_running_dataloadjob_not_stopped"));
				return result;
			}
		}
		...
		final ServicelayerJobModel jobModel = (ServicelayerJobModel) getCronJobService()
				.getJob(getDataLoaderCronJobParameters().getDataloadStartJobBeanId());
			...
			final SAPConfigurationModel configuration)
	{
		...
		{
			showErrorMessage(ctx, result);
		}
	}


	protected void showErrorMessage(final ActionContext<SAPConfigurationModel> ctx, final ActionResult<String> result)
	{
		result.setResultCode(ActionResult.ERROR);
