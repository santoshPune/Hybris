...
/**
 * Action node responsible for sourcing the order and allocating the
 * consignments. After the consignments are created, the consignment sub-process
 * is started for every consignment.
 */
public class SapSourceOrderAction extends AbstractProceduralAction<OrderProcessModel> {
	...
	public void executeAction(final OrderProcessModel process) throws RetryLaterException, Exception {
		...
		LOGGER.info(String.format("Proccess: %s in step %s", process.getCode(), getClass().getSimpleName()));
		...
		if (results != null) {
			...
			final Collection<ConsignmentModel> consignments = getAllocationService().createConsignments(
					process.getOrder(), "cons" + process.getOrder().getCode(), results);
	...
	private void logSourcingInfo(final SourcingResult result) {
		if (result != null) {
			...
			result.getAllocation().forEach(
					(product, qty) -> LOGGER.info(String.format("\tProduct [%s]: %s \tQuantity: '%d'", product
							.getProduct().getCode(), product.getProduct().getName(getSessionLocale()), qty)));
