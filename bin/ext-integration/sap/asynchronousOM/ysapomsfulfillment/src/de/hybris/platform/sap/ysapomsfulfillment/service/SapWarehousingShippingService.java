...
public class SapWarehousingShippingService extends DefaultWarehousingShippingService
{
	...
	public boolean isConsignmentConfirmable(final ConsignmentModel consignment)
	{
		...
		Assert.notNull(consignment.getOrder(), String.format("Order cannot be null for the SAP Consignment with code: [%s].", consignment.getCode()));
		final AbstractOrderModel order = consignment.getOrder();

		Assert.notNull(order.getStatus(), String.format("Order Status cannot be null for the Order with code: [%s].", order.getCode()));
		...
		Assert.isTrue(sapConsignmentProcess.isPresent(), String.format("No process found for the SAP Consignment with the code: [%s].", consignment.getCode()));
		...
		if (getValidConsConfirmConsignmentStatusList().contains(consignment.getStatus())
				...
				&& !sapConsignmentProcess.get().isDone()
