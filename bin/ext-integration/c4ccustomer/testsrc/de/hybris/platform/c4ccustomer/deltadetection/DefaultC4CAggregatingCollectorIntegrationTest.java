...
@IntegrationTest
public class DefaultC4CAggregatingCollectorIntegrationTest extends ServicelayerTest
{
	private final static Logger LOG = Logger.getLogger(DefaultC4CAggregatingCollectorIntegrationTest.class);
	...
	public void shouldConsumeEverything()
	{
		...
		changeDetectionService.collectChangesForType(typeService.getComposedTypeForClass(CustomerModel.class), streamId, collector);
		changeDetectionService.collectChangesForType(typeService.getComposedTypeForClass(AddressModel.class), streamId, collector);
	...
	public void shouldCreateSeparateInstances()
	{
		...
		assertFalse("The bean is singleton. Did you forget to define scope?", applicationContext
				.getBean("defaultC4CAggregatingCollector") == applicationContext.getBean("defaultC4CAggregatingCollector"));
	}
	...
	public void shouldGrabRelatedAddresses()
	{
		...
		address.setLine1("line 1");
		modelService.save(address);
		consumeAllChanges(streamId); // consume changes for customer and address, so next time both are not taken into account.

		// NON-1652: MySQL 5.5 does not support timestamps with milliseconds, so it will not compute the next change properly.
		// Thinking that the change was already consumed. (this will fail)

		// then we change customer, so the next collection should contain both, customer and address again.
		customer.setName("name");
		modelService.save(customer);
		final List<ItemChangeDTO> receivers = new ArrayList<>();

		// When we collect customer delta using defaultC4CDeltaCollector
		final C4CAggregatingCollector collector = getDeltaCollector(receivers);
		changeDetectionService.collectChangesForType(typeService.getComposedTypeForClass(CustomerModel.class), streamId, collector);

		// Then we have to get the address along with the customer
		assertThat(receivers, hasItem(hasProperty("itemPK", is(customer.getPk().getLongValue()))));
		assertThat(receivers, hasItem(hasProperty("itemPK", is(address.getPk().getLongValue()))));
	}
	...
	public void shouldGrabRelatedCustomers()
	{
		...
		// then we change address, so the next collection should contain both, customer and address again.
		address.setLine1("1");
		modelService.save(address);
	...
	public void shouldConsumeSyntheticChanges()
	{
		...
		changeDetectionService.collectChangesForType(typeService.getComposedTypeForClass(CustomerModel.class), streamId,
				tempCollector);
	...
	protected C4CAggregatingCollector getDeltaCollector(final List<ItemChangeDTO> receivers)
	{
		final C4CAggregatingCollector result = (C4CAggregatingCollector) applicationContext
				.getBean("defaultC4CAggregatingCollector");
	...
	protected void consumeAllChanges(final String streamId)
	{
		...
		changeDetectionService.collectChangesForType(typeService.getComposedTypeForClass(CustomerModel.class), streamId,
				tempCollector);
		changeDetectionService.collectChangesForType(typeService.getComposedTypeForClass(AddressModel.class), streamId,
				tempCollector);
		changeDetectionService.consumeChanges(tempCollector.getChanges());
	}

	protected void dump(final List<ItemChangeDTO> items)
	{
