...
import de.hybris.platform.customerticketingc4cintegration.data.*;
 ...
 */
public class C4CTicketFacadeImpl extends C4CBaseFacade implements TicketFacade
{
	...
	public TicketData createTicket(final TicketData ticket)
	{
		...
		try
		{
			...
			final ResponseEntity<String> result = restTemplate.postForEntity(Customerticketingc4cintegrationConstants.URL
	...
	public SearchPageData<TicketData> getTickets(final PageableData pageableData)
	{
		...
		final String sorting = StringUtils.isNotBlank(pageableData.getSort()) ? pageableData.getSort() : Customerticketingc4cintegrationConstants.ORDER_DEFAULT_VALUE;
		...
		final UriComponentsBuilder builder = UriComponentsBuilder
				...
				.queryParam(Customerticketingc4cintegrationConstants.PAGING_SKIP_SUFFIX, skip)
				.queryParam(Customerticketingc4cintegrationConstants.PAGING_TOP_SUFFIX, top)
		...
		try
		{
			...
			final ODataListResultsData oDataListResultsData = jacksonObjectMapper.readValue(result.getBody(), ODataListResponseData.class).getD();

			LOGGER.info(Integer.parseInt(oDataListResultsData.get__count()));
			return convertPageData(oDataListResultsData.getResults(), ticketConverter, pageableData, Integer.parseInt(oDataListResultsData.get__count()));
		}
		catch (final IOException e)
		{
		...
		return convertPageData(Collections.EMPTY_LIST, ticketConverter, pageableData, 0);
	}
