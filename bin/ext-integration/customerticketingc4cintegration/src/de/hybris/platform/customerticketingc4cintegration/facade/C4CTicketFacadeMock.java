 ...
 */
public class C4CTicketFacadeMock extends C4CBaseFacade implements TicketFacade
{
	...
	public SearchPageData<TicketData> getTickets(final PageableData pageableData)
	{
		...
		try
		{
			final List<ServiceRequestData> results = jacksonObjectMapper.readValue(json, ODataListResponseData.class).getD()
					.getResults();
			return convertPageData(results, ticketConverter, pageableData, results.size());
		}
		catch (final IOException e)
		{
			LOGGER.warn(e);
