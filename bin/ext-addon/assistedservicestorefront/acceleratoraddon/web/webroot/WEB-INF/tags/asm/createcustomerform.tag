...
<form action="${createAccountActionUrl}" method="post" id="_asmCreateAccountForm" class="asmForm">
	...
	<button type="submit" class="ASM-btn ASM-btn-create-account hidden" disabled><spring:theme code="asm.emulate.createAccount"/><span class="hidden-xs hidden-sm"><spring:theme code="asm.emulate.createAccount.ending"/></span></button>
</form>