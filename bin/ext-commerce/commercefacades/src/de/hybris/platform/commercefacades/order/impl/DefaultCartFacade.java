...
public class DefaultCartFacade implements CartFacade
{
	...
	public CartModificationData updateCartEntry(final OrderEntryData orderEntry) throws CommerceCartModificationException
	{
		...
		if (CollectionUtils.isEmpty(orderEntry.getEntries()))
		{
			...
			if (CollectionUtils.isNotEmpty(orderEntry.getConfigurationInfos()))
			{
	...
	protected void mergeOrderEntryWithModelConfiguration(final OrderEntryData orderEntry)
	{
				...
				.entrySet())
		{
			...
			mergedConfiguration.addAll(
					mergeStrategyEntry.getValue().merge(
							filterConfigurationsByConfigurator(orderEntryConfiguration, configuratorType),
		...
		final ConfiguratorType configuratorEnum)
	{
		return configurations.stream()
				.filter(item -> item.getConfiguratorType() == configuratorEnum)
			...
			throws CommerceCartModificationException
	{
        ...
        List<ProductConfigurationItem> productConfigurationItemList =  orderEntry.getConfigurationInfos().stream()
                .map(this::configurationInfoToProductConfiguration).collect(Collectors.toList());

        CommerceCartParameter commerceCartParameter = createCommerceCartParameter(
                orderEntry,
                productConfigurationItemList);
	...
	public void setOrderEntryConverter(
			final Converter<AbstractOrderEntryModel, OrderEntryData> orderEntryConverter)
	{
