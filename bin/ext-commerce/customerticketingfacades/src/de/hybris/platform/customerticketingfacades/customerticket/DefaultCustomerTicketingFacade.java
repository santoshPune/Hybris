...
public class DefaultCustomerTicketingFacade implements TicketFacade
{
	...
	{
		final SearchPageData<CsTicketModel> searchPageData = ticketService.getTicketsForCustomerOrderByModifiedTime(
				userService.getCurrentUser(), baseSiteService.getCurrentBaseSite(), pageableData);

		return convertPageData(searchPageData, ticketConverter);
	}
	...
	private CsTicketParameter createCsTicketParameter(final TicketData ticketData)
	{
		final CsTicketParameter ticketParameter = new CsTicketParameter();
		ticketParameter.setPriority(enumerationService.getEnumerationValue(CsTicketPriority._TYPECODE, ticketPriority));
		ticketParameter.setReason(enumerationService.getEnumerationValue(CsEventReason._TYPECODE, ticketReason));
		ticketParameter.setAssociatedTo(ticketService.getAssociatedObject(ticketData.getAssociatedTo(), null, null));
