...
public class AcceleratorSaveCartFacadeIntegrationTest extends ServicelayerTransactionalTest
{
	...
	@Test
	public void testSaveCartHooks() throws CommerceSaveCartException
	{
		// enable hook
		configuration.setProperty("acceleratorservices.commercesavecart.sessioncart.hook.enabled", "true");
		String originalSessionCartId = cartService.getSessionCart().getCode();
		saveSessionCart();

		assertFalse(originalSessionCartId.equals(cartService.getSessionCart().getCode()));

		// disable hook
		configuration.setProperty("acceleratorservices.commercesavecart.sessioncart.hook.enabled", "false");
		originalSessionCartId = cartService.getSessionCart().getCode();
		saveSessionCart();

		assertTrue(originalSessionCartId.equals(cartService.getSessionCart().getCode()));
	}

	@Test
	public void testRestoreSaveCartHook() throws CommerceSaveCartException
	{
		// enable hook
		configuration.setProperty("acceleratorservices.commercesavecart.restoration.savetime.hook.enabled", "true");

		// save session cart
		CommerceSaveCartParameterData parameter = saveSessionCart();
		// restore saved session cart
		saveCartFacade.restoreSavedCart(parameter);
		assertNull(cartService.getSessionCart().getSaveTime());

		// disable hook
		configuration.setProperty("acceleratorservices.commercesavecart.restoration.savetime.hook.enabled", "false");
		// save session cart again
		parameter = saveSessionCart();
		// restore saved session cart
		saveCartFacade.restoreSavedCart(parameter);
		assertNotNull(cartService.getSessionCart().getSaveTime());
	}
